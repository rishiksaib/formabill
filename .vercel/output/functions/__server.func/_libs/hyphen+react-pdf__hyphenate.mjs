...
//#endregion
//#region node_modules/@react-pdf/hyphenate/lib/index.js
var import_en_us = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(root, factory) {
		if (typeof define === "function" && define.amd) define([], factory);
		else if (typeof module === "object" && module.exports) module.exports = factory();
		else root.hyphenationPatternsEnUs = factory();
	})(exports, function() {
		return {
			patterns: [
				" ",
...
var addPattern = (root, source) => {
	...
	} else if (node.plain) node.plain.push(entry);
	else node.plain = [entry];
};
var SCRATCH_LIMIT = 128;
var scratch = /* @__PURE__ */ new Uint8Array(129);
/**
...
* @returns Hyphenator
*/
var createHyphenator = (source) => {
	const root = createNode();
	for (let i = 0; i < source.patterns.length; i += 1) addPattern(root, source.patterns[i]);
	...
	const splitWord = (word) => {
		...
		for (let start = 0; start < length; start += 1) {
			...
			for (let end = start; end < length; end += 1) {
				...
				if (plain) for (let p = 0; p < plain.length; p += 1) {
				...
				if (anchored) for (let p = 0; p < anchored.length; p += 1) {
	...
	*/
	const splitToken = (token) => {
		...
		while (index < token.length) {
			...
			let end = index + 1;
			while (end < token.length && !NON_LETTER.test(token[end]) === isLetter) end += 1;
			const chunk = token.slice(index, end);
			if (isLetter) {
				const chunkParts = splitWord(chunk);
				parts[parts.length - 1] += chunkParts[0];
				for (let i = 1; i < chunkParts.length; i += 1) parts.push(chunkParts[i]);
...
//#endregion
//#region node_modules/@react-pdf/hyphenate/lib/en-us.js
var { hyphenate, syllables } = createHyphenator(import_en_us.default);
//#endregion
export { syllables as t };
