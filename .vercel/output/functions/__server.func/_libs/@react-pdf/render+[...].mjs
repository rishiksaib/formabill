...
import { C as Rect, D as TextInstance, E as TextInput, O as Tspan, S as RadialGradient, T as Text, _ as Note, b as Polygon, c as FieldSet, d as ImageBackground, f as Line, h as List, i as Circle, m as Link, n as Canvas, p as LinearGradient, r as Checkbox, s as Ellipse, u as Image, w as Select, x as Polyline, y as Path } from "./image+[...].mjs";
import { c as isNil, d as matchPercent } from "../react-pdf__fns.mjs";
import { n as cs } from "./layout+[...].mjs";
//#region node_modules/abs-svg-path/index.js
...
//#endregion
//#region node_modules/parse-svg-path/index.js
...
//#endregion
//#region node_modules/normalize-svg-path/index.mjs
	...
	for (var i = 0, len = path.length; i < len; i++) {
		...
		switch (command) {
			...
			case "V":
				...
				break;
			case "Z": seg = line(x, y, startX, startY);
		}
		prev = command;
		x = seg[seg.length - 2];
		y = seg[seg.length - 1];
...
//#endregion
//#region node_modules/@react-pdf/render/lib/index.js
var expandSmoothQuadratics = (d) => {
	const segments = (0, import_abs_svg_path.default)((0, import_parse_svg_path.default)(d));
	let x = 0;
	let y = 0;
	let startX = 0;
	let startY = 0;
	...
	return segments.map((segment) => {
		...
		if (command === "H") x = seg[1];
		else if (command === "V") y = seg[1];
		else if (command === "Z") {
			x = startX;
			y = startY;
		} else {
		...
		return seg;
	}).map((seg) => seg[0] + seg.slice(1).join(" ")).join("");
};
...
var renderRun = (ctx, run) => {
	...
	ctx.fillOpacity(opacity);
	if (link) {
		if (isSrcId$1(link)) ctx.goTo(0, -height - descent, xAdvance, height, link.slice(1));
...
var setDestination = (ctx, node) => {
	if (!node.box) return;
	if (!node.props) return;
	if ("id" in node.props) ctx.addNamedDestination(node.props.id, "XYZ", node.box.left ?? 0, node.box.top, null);
};
var clean = (options) => {
	const opt = { ...options };
...
*/
var addFormAnnotation = (ctx, name, type, box, options, extra) => {
	...
	Object.entries(extra).forEach(([key, value]) => {
		if (value !== void 0) dict[key] = value;
	});
...
var renderCheckbox = (ctx, node, options) => {
	...
	addFormAnnotation(ctx, name, "checkbox", node.box, fieldOptions, {
		AP,
		AS
	});
};
