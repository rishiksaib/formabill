...
import { a as invoiceSubtotal, c as isoDate, d as toPaise, f as uid, o as invoiceTax, s as invoiceTotal, t as addFrequency, u as nextInvoiceNumber } from "./utils-DgMi_UEP.mjs";
...
import { a as listInvoices, i as getInvoice, n as deleteInvoice, o as markInvoicePaid, r as findByPaymentLinkId, s as upsertInvoice, t as countInvoicesForMonth } from "./server-store.server-L3sWWelb.mjs";
...
import { t as nodemailer } from "../_libs/nodemailer.mjs";
...
import { i as hkdf, r as sha256 } from "../_libs/noble__hashes.mjs";
...
import { a as hexToBytes, i as bytesToHex, o as managedNonce, r as xchacha20poly1305, s as utf8ToBytes } from "../_libs/noble__ciphers.mjs";
import { n as defu, t as createDefu } from "../_libs/defu.mjs";
import { t as Pool } from "../_libs/pg.mjs";
import { createHash, createHmac, randomBytes, timingSafeEqual } from "node:crypto";
//#region node_modules/.nitro/vite/services/ssr/assets/types-DaKPqAA5.js
...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/router-DBCVdn-Z.js
...
var EmailNotConfiguredError = class extends Error {
	constructor() {
		super("Password reset email is not configured. Set SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, and SMTP_FROM, then try again.");
...
function mailConfigured() {
	return Boolean(smtpEnv("SMTP_HOST") && smtpEnv("SMTP_PORT") && smtpEnv("SMTP_USER") && smtpEnv("SMTP_PASS") && smtpEnv("SMTP_FROM"));
}
...
async function sendPasswordResetEmail(to, resetUrl) {
	await sendMail(to, "Reset your FormaBill password", [
...
var styles_default = "/assets/styles-C1AxQdTN.css";
var APP_NAME = "FormaBill";
var APP_DESCRIPTION = "Fast pro invoices for freelancers worldwide. Share on WhatsApp, get paid via UPI, PayPal, or your gateway. FormaBill takes no cut of client payments.";
var Route$30 = createRootRoute({
	head: () => ({
		meta: [
			...
			{
				property: "og:title",
				content: `${APP_NAME} — Fast pro invoices for freelancers worldwide`
			},
			{
...
var $$splitComponentImporter$5 = () => import("./terms-rHiJAHm2.mjs");
...
var _0002_user_invoice_quota_default = "alter table \"user\" add column if not exists \"isPro\" boolean not null default false;\r\n";
var _0003_mcp_tokens_default = "create table if not exists \"mcp_tokens\" (\n  \"id\" text not null primary key,\n  \"userId\" text not null references \"user\" (\"id\") on delete cascade,\n  \"name\" text not null,\n  \"tokenHash\" text not null unique,\n  \"prefix\" text not null,\n  \"createdAt\" timestamptz default CURRENT_TIMESTAMP not null,\n  \"lastUsedAt\" timestamptz,\n  \"revokedAt\" timestamptz\n);\n\ncreate index if not exists \"mcp_tokens_user_idx\" on \"mcp_tokens\" (\"userId\");\n";
var _0004_studio_settings_default = "create table if not exists \"studio_settings\" (\n  \"userId\" text not null primary key references \"user\" (\"id\") on delete cascade,\n  \"data\" jsonb not null default '{}',\n  \"updatedAt\" timestamptz default CURRENT_TIMESTAMP not null\n);\n";
var _0005_mcp_clients_default = "create table if not exists \"mcp_clients\" (\n  \"id\" text not null primary key,\n  \"userId\" text not null references \"user\" (\"id\") on delete cascade,\n  \"name\" text not null,\n  \"email\" text not null default '',\n  \"createdAt\" timestamptz default CURRENT_TIMESTAMP not null\n);\n\ncreate index if not exists \"mcp_clients_user_idx\" on \"mcp_clients\" (\"userId\");\n";
var _0006_user_lifetime_pro_default = "alter table \"user\" add column if not exists \"isLifetimePro\" boolean not null default false;\n";
var _0007_user_pro_plan_default = "alter table \"user\" add column if not exists \"proPlan\" text;\nalter table \"user\" add column if not exists \"proExpiresAt\" timestamptz;\n";
var _0008_gift_codes_default = "create table if not exists \"gift_codes\" (\n  \"id\" text not null primary key,\n  \"code\" text not null unique,\n  \"createdBy\" text not null references \"user\" (\"id\") on delete cascade,\n  \"durationMonths\" integer not null,\n  \"expiresAt\" timestamptz not null,\n  \"redeemedBy\" text references \"user\" (\"id\") on delete set null,\n  \"redeemedAt\" timestamptz,\n  \"createdAt\" timestamptz default CURRENT_TIMESTAMP not null\n);\n\ncreate index if not exists \"gift_codes_creator_idx\" on \"gift_codes\" (\"createdBy\");\n";
...
async function createPgliteSql() {
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({
			...
			"/migrations/0002_user_invoice_quota.sql": _0002_user_invoice_quota_default,
			"/migrations/0003_mcp_tokens.sql": _0003_mcp_tokens_default,
			"/migrations/0004_studio_settings.sql": _0004_studio_settings_default,
			"/migrations/0005_mcp_clients.sql": _0005_mcp_clients_default,
			"/migrations/0006_user_lifetime_pro.sql": _0006_user_lifetime_pro_default,
			"/migrations/0007_user_pro_plan.sql": _0007_user_pro_plan_default,
...
var auth = betterAuth({
	...
	emailAndPassword: {
		...
		sendResetPassword: async ({ user, url }) => {
			const { sendPasswordResetEmail } = await import("./mailer.server-BeqSy-Dx.mjs");
...
*/
async function requireFreeTierUserId(bearerToken) {
...
*   when this is false.
*/
var Route$22 = createFileRoute("/api/auth-status")({ server: { handlers: { GET: ({ request }) => {
...
function credentials(overrides) {
	const keyId = overrides?.keyId?.trim() || env$2("RAZORPAY_KEY_ID");
	const keySecret = overrides?.keySecret?.trim() || env$2("RAZORPAY_KEY_SECRET");
...
function appBaseUrl(request) {
	const fromEnv = env$2("NEXT_PUBLIC_APP_URL") || env$2("APP_URL") || env$2("VITE_PUBLIC_APP_URL");
	if (fromEnv) return fromEnv.replace(/\/$/, "");
	const vercelUrl = env$2("VERCEL_URL");
	...
	if (request) {
		...
		if (forwarded) {
			...
			if (host && !/^localhost(:\d+)?$/i.test(host) && host !== "127.0.0.1:8080") return `${proto}://${host}`;
		}
		const host = request.headers.get("host");
		if (host && !/^(localhost|127\.0\.0\.1|\[::1\])(:\d+)?$/i.test(host.trim())) return `https://${host.trim()}`;
	}
	return "http://127.0.0.1:8080";
}
async function createPaymentLink(invoice, request, userCredentials) {
	if (Boolean(userCredentials?.keyId || userCredentials?.keySecret) && (!userCredentials?.keyId || !userCredentials?.keySecret)) throw new Error("Enter both Razorpay Key ID and Key Secret, or clear both fields.");
	const creds = credentials(userCredentials);
	const amount = toPaise(invoiceTotal(invoice.lineItems, invoice.taxRate));
	if (amount < 100) throw new Error("Invoice total must be at least ₹1.00 to collect payment.");
	if (!creds) return {
		demo: true,
		message: "Razorpay is not configured. Add UPI or bank details, or connect Razorpay in Settings."
	};
	...
	const body = {
		...
		customer: {
			...
			email: invoice.client.email || invoice.fromEmail || void 0
		},
		notify: {
			email: Boolean(invoice.client.email),
			sms: false
		},
	...
	if (!res.ok || !json.short_url) throw new Error(json.error?.description || "Razorpay could not create a payment link.");
	return {
		demo: false,
		id: json.id,
...
function verifyWebhookSignature(rawBody, signature) {
	const secret = env$2("RAZORPAY_WEBHOOK_SECRET");
...
async function requireRequestUserId() {
...
*/
async function getFreeTierUserId() {
	try {
...
async function requireFreeTierRequestUserId() {
...
function checkRateLimit(key, limit, windowMs, now = Date.now()) {
	...
	return {
		allowed: false,
		retryAfterSec: Math.max(1, Math.ceil((bucket.resetAt - now) / 1e3))
	};
}
...
function rateLimitedResponse(retryAfterSec) {
	return Response.json({ error: `Too many requests. Please wait ${retryAfterSec}s and try again.` }, {
		status: 429,
		headers: { "Retry-After": String(retryAfterSec) }
	});
}
var Route$21 = createFileRoute("/api/checkout")({ server: { handlers: { POST: async ({ request }) => {
	try {
		const limit = checkRateLimit(`checkout:POST:${clientIp(request)}`, 20, 6e4);
		if (!limit.allowed) return rateLimitedResponse(limit.retryAfterSec);
		const body = await request.json();
		if (!body.invoiceId) return Response.json({ error: "invoiceId is required" }, { status: 400 });
		...
		if (!invoice) return Response.json({ error: "Invoice not found. Publish it first." }, { status: 404 });
		if (body.confirmDemo) {
			if (razorpayConfigured()) return Response.json({ error: "Demo pay is disabled when Razorpay is configured." }, { status: 400 });
		...
		if (link.demo) return Response.json({
			demo: true,
		...
		return Response.json({ error: err instanceof Error ? err.message : "Checkout failed" }, { status: 400 });
	}
} } } });
/** A Pro "month" is 30 days for gift/reward grants. */
var PRO_MONTH_MS = 2592e6;
/**
...
*/
function canGiftForSource(source, storedCanGift) {
...
async function grantProDays(userId, days) {
  ...
  `;
	...
	const expiresAt = new Date((Number.isFinite(current) ? Math.max(current, Date.now()) : Date.now()) + days * 24 * 60 * 60 * 1e3).toISOString();
...
*/
async function grantProMonths(userId, months) {
	const sql = await getSql();
	const rows = await sql`
    select "proExpiresAt" from "user" where "id" = ${userId} limit 1
  `;
	const current = rows[0]?.proExpiresAt != null ? Date.parse(String(rows[0].proExpiresAt)) : NaN;
	const expiresAt = new Date((Number.isFinite(current) ? Math.max(current, Date.now()) : Date.now()) + months * PRO_MONTH_MS).toISOString();
	await sql`update "user" set "proExpiresAt" = ${expiresAt}, "updatedAt" = CURRENT_TIMESTAMP where "id" = ${userId}`;
	return expiresAt;
}
...
function randomReferralCode() {
	...
	let code = "";
	for (const byte of bytes) code += CODE_ALPHABET$1[byte % 31];
	return code;
}
...
*/
async function attributeReferral(userId, input) {
...
*/
async function maybeRewardReferrer(referredUserId) {
	const sql = await getSql();
	const row = (await sql`
  ...
  `)[0];
	if (!row) return false;
	await grantProMonths(row.referrerUserId, 1);
...
var GIFT_GRANTS = {
	pro_monthly: {
		days: 7,
		label: "7 days Pro",
		quotaLine: "1 friend · 7 days Pro · 1 code/month"
	},
	pro_yearly: {
		days: 30,
		label: "1 month Pro",
...
async function giftGrantFor(userId) {
	...
	return {
		days: GIFT_GRANTS.pro_monthly.days,
		plan: "pro_monthly"
	};
}
var GiftQuotaExhaustedError = class extends Error {
	status = 403;
	constructor(message) {
...
async function listGiftCodes(userId) {
	return (await (await getSql())`
...
*/
var Route$20 = createFileRoute("/api/gift-codes")({ server: { handlers: {
	GET: async () => {
		try {
			...
			if (error instanceof GiftProRequiredError) return Response.json({
				codes: [],
				canGift: false,
				giftsRemaining: 0,
				proSource: null,
			...
			return Response.json({ error: error instanceof Error ? error.message : "Could not load gift codes" }, { status: 400 });
		}
	},
	POST: async ({ request }) => {
		try {
...
var Route$19 = createFileRoute("/api/invoices")({ server: { handlers: {
	GET: async ({ request }) => {
		try {
			const userId = await requireFreeTierRequestUserId();
	...
	POST: async ({ request }) => {
		try {
			const limit = checkRateLimit(`invoices:POST:${clientIp(request)}`, 30, 6e4);
			if (!limit.allowed) return rateLimitedResponse(limit.retryAfterSec);
			const userId = await requireFreeTierRequestUserId();
			const body = await request.json();
			if (!body?.id || !body.number) return Response.json({ error: "Invoice id and number are required" }, { status: 400 });
			const existing = await getInvoice(body.id, userId);
			if (userId && !existing && !await isProUser(userId) && await countInvoicesForMonth(userId) >= 5) return Response.json({ error: "Free plan limit reached: 5 invoices per month. Upgrade to Pro to continue." }, { status: 402 });
			...
			return Response.json({ error: err instanceof Error ? err.message : "Could not save invoice" }, { status: 400 });
		}
	}
} } });
var MCP_TOKEN_PREFIX = "fb_mcp_";
var MCP_UPGRADE_MESSAGE = "AI connection is a Pro feature. Upgrade to Pro to generate MCP tokens.";
var MCP_PAID_ONLY_MESSAGE = "MCP access needs a paid Pro subscription — gift and trial grants don't include AI connection.";
...
var McpProRequiredError = class extends Error {
	status = 402;
	constructor() {
...
*/
var McpPaidOnlyError = class extends Error {
	status = 403;
	constructor() {
...
*/
async function requireMcpAccess(userId) {
	const plan = await getUserPlan(userId);
...
*/
async function createMcpToken(userId, name) {
...
async function revokeMcpToken(userId, id) {
	return (await (await getSql())`
...
async function authenticateMcpToken(token) {
  ...
  `)[0];
	...
	if (!plan.canGift && plan.proSource !== "subscription") return {
		ok: false,
		reason: "paid_only"
	};
	sql`update "mcp_tokens" set "lastUsedAt" = CURRENT_TIMESTAMP where "id" = ${row.id}`.catch(() => void 0);
	return {
		ok: true,
		tokenId: row.id,
		userId: row.userId
...
function createDraftInvoice(settings = DEFAULT_SETTINGS, existingNumbers = []) {
	return {
		id: uid(),
...
function applySettingsToInvoice(invoice, settings) {
	return {
		...invoice,
...
async function getStudioSettings(userId) {
	const stored = (await (await getSql())`
...
async function updateStudioSettings(userId, patch) {
	const current = await getStudioSettings(userId);
	const next = {
		...current,
		...Object.fromEntries(Object.entries(patch).filter(([, value]) => value !== void 0)),
		paymentMethods: {
...
function publicStudioSettings(settings) {
...
async function listServerClients(userId) {
	return await (await getSql())`
...
async function saveServerClient(userId, input) {
	const name = input.name.trim();
	const email = (input.email ?? "").trim();
	if (!name) throw new Error("Client name is required.");
	const sql = await getSql();
	const existing = email ? await sql`
        select "id", "name", "email", "createdAt" from "mcp_clients"
...
function publicInvoiceUrl(id) {
	return `${appBaseUrl()}/inv/${encodeURIComponent(id)}`;
}
function money(amount, currency) {
	return `${currency === "INR" ? "₹" : currency === "EUR" ? "€" : currency === "GBP" ? "£" : "$"}${amount.toLocaleString("en-US", { maximumFractionDigits: 2 })}`;
}
/** Human first line + machine JSON, so the assistant can quote and parse. */
function summary(human, data) {
	return { content: [{
		type: "text",
...
function asString(value, field, required = false) {
...
var lineItemSchema = {
	...
	properties: {
		description: {
			type: "string",
			description: "Line description, e.g. 'Brand identity'"
		},
		quantity: {
			type: "number",
			default: 1
...
function toLineItems(value) {
	if (!Array.isArray(value) || value.length === 0) throw new Error("\"lineItems\" must be a non-empty array.");
...
var TOOLS = [
	{
		...
		run: async (userId, raw) => {
			...
			return (status ? all.filter((inv) => inv.status === status) : all).slice(0, limit).map(invoiceSummary);
		}
	},
	{
		name: "get_invoice",
		...
		run: async (userId, raw) => {
			...
			if (!invoice) throw new Error("Invoice not found. Use list_invoices to find the id (pass the \"id\", not the number, when both exist).");
			return {
				...invoice,
				totals: {
	...
	{
		name: "create_invoice",
		description: "Create a DRAFT invoice for the user's client — never sent, never charged. From-name, currency, tax, and payment methods default from studio settings. Use update_invoice_status to send it, and get_invoice_public_link to share it.",
		inputSchema: {
			type: "object",
			...
			properties: {
				...
				currency: {
					type: "string",
					enum: [...STUDIO_CURRENCIES]
				},
				taxRate: { type: "number" },
				dueDate: {
					type: "string",
					description: "ISO date, defaults to 14 days out"
				},
				notes: { type: "string" }
			}
		},
		run: async (userId, raw) => {
			...
			if (!STUDIO_CURRENCIES.includes(currency)) throw new Error(`"currency" must be one of ${STUDIO_CURRENCIES.join(", ")}.`);
			const saved = await upsertInvoice({
				...draft,
				id: draft.id,
			...
			return summary(`Created invoice ${saved.number} for ${clientName || "the client"}, ${money(created.total, created.currency)} (draft). Public link: ${publicInvoiceUrl(saved.id)}`, created);
		}
	},
	{
		name: "update_invoice_status",
		description: "Move an invoice between draft, sent, and paid. Pass the status explicitly — marking paid means the client has already paid by hand; it never charges anyone.",
		inputSchema: {
			type: "object",
			required: ["id", "status"],
			properties: {
		...
		run: async (userId, raw) => {
			...
			if (status !== "draft" && status !== "sent" && status !== "paid") throw new Error("\"status\" must be exactly draft, sent, or paid.");
			const existing = await getInvoice(id, userId);
			if (!existing) throw new Error("Invoice not found. Use list_invoices to find the \"id\".");
			const saved = await upsertInvoice({
				...existing,
				status,
				userId
			...
			return summary(`Invoice ${saved.number} is now ${status}. Total ${money(result.total, result.currency)}.`, result);
		}
	},
	{
		name: "get_invoice_public_link",
		description: "Get the shareable public page URL for an invoice (publishes a working link without changing its status). Send this URL to the client.",
		inputSchema: {
			type: "object",
			required: ["id"],
			properties: { id: {
		...
		run: async (userId, raw) => {
			const id = asString(asRecord(raw).id, "id", true);
			...
			return summary(`Public link for invoice ${saved.number} (${saved.status}): ${url}`, {
				id: saved.id,
				number: saved.number,
				status: saved.status,
				url
	...
	},
	{
		...
		run: async (userId, raw) => {
			...
			if (args.client !== void 0) {
				...
				next.client = {
					...existing.client,
					...client.name !== void 0 ? { name: asString(client.name, "client.name", true) } : {},
					...client.email !== void 0 ? { email: asString(client.email, "client.email") } : {}
				};
			}
			if (args.lineItems !== void 0) next.lineItems = toLineItems(args.lineItems);
			if (args.currency !== void 0) {
				const currency = asString(args.currency, "currency", true);
				if (!STUDIO_CURRENCIES.includes(currency)) throw new Error(`"currency" must be one of ${STUDIO_CURRENCIES.join(", ")}.`);
				next.currency = currency;
			}
			if (args.taxRate !== void 0) next.taxRate = asNumber(args.taxRate, "taxRate");
			if (args.notes !== void 0) next.notes = asString(args.notes, "notes");
			if (args.dueDate !== void 0) next.dueDate = asString(args.dueDate, "dueDate", true);
			if (args.status !== void 0) {
				const status = asString(args.status, "status", true);
				if (status !== "draft" && status !== "sent" && status !== "paid") throw new Error("\"status\" must be draft, sent, or paid.");
				next.status = status;
			}
			return invoiceSummary(await upsertInvoice(next, userId));
		}
	},
	{
		name: "mark_invoice_paid",
		description: "Record that the client already paid by hand (cash, UPI, bank). Bookkeeping only — it never moves money.",
		inputSchema: {
			type: "object",
			required: ["id"],
			properties: { id: {
		...
		run: async (userId, raw) => {
			...
			return summary(`Invoice ${result.number} marked paid. Total ${money(result.total, result.currency)} — no money moved, record only.`, result);
		}
	},
	{
		name: "create_payment_link",
		description: "Create a Razorpay payment link so the CLIENT can pay online. This is the ONLY tool that starts a real money flow — confirm with the user first. Uses the studio's stored Razorpay keys unless keyId/keySecret are passed. Without keys, returns guidance instead of a link (nothing is charged).",
		inputSchema: {
			type: "object",
			required: ["invoiceId"],
			properties: {
				...
				keyId: {
					type: "string",
					description: "Razorpay Key ID (overrides stored keys)"
				},
				keySecret: {
					type: "string",
					description: "Razorpay Key Secret (overrides stored keys)"
				}
			}
		},
		run: async (userId, raw) => {
			...
			const settings = await getStudioSettings(userId);
			const link = await createPaymentLink(invoice, void 0, {
			...
			if (link.demo) return summary(link.message ?? "No payment link created — add UPI in Settings or connect Razorpay first. Nothing was charged.", {
				demo: true,
				invoice: invoiceSummary(invoice)
			});
			const result = invoiceSummary(await upsertInvoice({
				...invoice,
				userId,
				status: invoice.status === "paid" ? "paid" : "sent",
				paymentLinkId: link.id,
			...
			return summary(`Payment link created for invoice ${result.number} (${money(result.total, result.currency)}): ${link.shortUrl} — send it to the client to collect.`, {
				demo: false,
				id: link.id,
	...
	},
	{
		...
		run: async (userId) => {
			const [stored, invoices] = await Promise.all([listServerClients(userId), listInvoices(userId)]);
			const seen = /* @__PURE__ */ new Map();
			for (const client of stored) seen.set(client.email ? `e:${client.email.toLowerCase()}` : `n:${client.name}`, {
				name: client.name,
				email: client.email
			});
	...
	{
		name: "upsert_client",
		description: "Save a client for reuse on future invoices. Same email twice updates the existing client instead of duplicating.",
	...
	},
	{
		...
		description: "Update the studio profile, invoice defaults, or payment methods. Only provided fields change.",
		inputSchema: {
			type: "object",
			properties: {
				name: { type: "string" },
		...
		run: async (userId, raw) => {
			...
			for (const field of [
				...
				"email",
				"address"
			]) if (args[field] !== void 0) patch[field] = asString(args[field], field);
			if (args.defaultCurrency !== void 0) {
				const currency = asString(args.defaultCurrency, "defaultCurrency", true);
				if (!STUDIO_CURRENCIES.includes(currency)) throw new Error(`"defaultCurrency" must be one of ${STUDIO_CURRENCIES.join(", ")}.`);
				patch.defaultCurrency = currency;
			}
			if (args.defaultTaxRate !== void 0) {
			...
			for (const field of paymentFields) if (args[field] !== void 0) paymentMethods[field] = asString(args[field], field);
			return publicStudioSettings(await updateStudioSettings(userId, {
				...patch,
...
async function callMcpTool(userId, name, args) {
	const tool = TOOLS.find((t) => t.name === name || t.aliases?.includes(String(name)));
	if (!tool) return fail(`Unknown tool "${String(name)}". Available tools: ${TOOLS.map((t) => t.name).join(", ")}.`);
	try {
...
var LATEST_PROTOCOL = "2025-06-18";
var SERVER_INFO = {
	name: "formabill",
	version: "1.0.0"
};
...
async function handleOne(request, userId, raw) {
	if (!raw || typeof raw !== "object" || Array.isArray(raw)) return error(null, -32600, "Invalid Request: expected a JSON-RPC object.");
	const { jsonrpc, id = null, method, params } = raw;
	if (jsonrpc !== "2.0" || typeof method !== "string") return error(id ?? null, -32600, "Invalid Request: bad jsonrpc version or method.");
	if (method === "notifications/initialized") return new Response(null, { status: 202 });
	const requestId = id ?? null;
	if (method === "initialize") {
		const requested = params && typeof params === "object" && !Array.isArray(params) ? params.protocolVersion : void 0;
		return response(requestId, {
			protocolVersion: typeof requested === "string" && SUPPORTED_PROTOCOLS.includes(requested) ? requested : LATEST_PROTOCOL,
			capabilities: { tools: {} },
	...
	if (method === "tools/call") {
		...
		if (typeof args.name !== "string" || !args.name) return error(requestId, -32602, "Invalid params: \"name\" is required.");
		return response(requestId, await callMcpTool(userId, args.name, args.arguments));
	}
	return error(requestId, -32601, `Method not found: ${method}.`);
}
var Route$18 = createFileRoute("/api/mcp")({ server: { handlers: {
	GET: () => Response.json({ error: "FormaBill MCP endpoint. Connect with Streamable HTTP: POST JSON-RPC to this URL with an Authorization: Bearer <token> header." }, {
		status: 405,
		headers: {
			Allow: "POST",
			...corsHeaders()
	...
	POST: async ({ request }) => {
		...
		if (!token) return httpError("Missing MCP token. Send Authorization: Bearer <token> (Settings → AI / MCP).", 401);
		const auth = await authenticateMcpToken(token).catch(() => null);
		if (!auth || !auth.ok) return httpError(auth?.reason === "not_pro" ? MCP_UPGRADE_MESSAGE : auth?.reason === "paid_only" ? MCP_PAID_ONLY_MESSAGE : "Invalid or revoked MCP token. Generate a new one in Settings → AI / MCP.", auth?.reason === "not_pro" || auth?.reason === "paid_only" ? 403 : 401);
		const limit = checkRateLimit(`mcp:${auth.tokenId}`, 100, 6e4);
		if (!limit.allowed) {
		...
		if (Array.isArray(body)) {
			const settled = await Promise.all(body.map(async (entry) => {
				const res = await handleOne(request, auth.userId, entry);
				if (!res) return null;
				if (res.status === 202) return null;
				return res.json();
			}));
			return Response.json(settled.filter((entry) => entry !== null), { headers: corsHeaders() });
		}
...
var $$splitComponentImporter$1 = () => import("./settings-c0yh0os5.mjs");
...
var $$splitComponentImporter = () => import("./inv._id-DM-yIvs6.mjs");
...
var Route$11 = createFileRoute("/api/gift-codes/redeem")({ server: { handlers: { POST: async ({ request }) => {
	try {
		const limit = checkRateLimit(`gift-redeem:POST:${clientIp(request)}`, 20, 6e4);
		if (!limit.allowed) return rateLimitedResponse(limit.retryAfterSec);
		const userId = await requireRequestUserId();
		const body = await request.json().catch(() => ({}));
		const result = await redeemGiftCode(userId, typeof body.code === "string" ? body.code : "");
		return Response.json({
			ok: true,
			...result
		});
	} catch (error) {
		if (error instanceof UnauthorizedError) return Response.json({ error: "Sign in to redeem a gift code — Pro lands on your account." }, { status: 401 });
		return Response.json({ error: error instanceof Error ? error.message : "Could not redeem code" }, { status: 400 });
	}
} } } });
var Route$10 = createFileRoute("/api/invoices/$id")({ server: { handlers: {
	...
	PUT: async ({ params, request }) => {
		try {
			...
			if (!merged.number || !merged.client || !merged.lineItems) return Response.json({ error: "Invalid invoice" }, { status: 400 });
...
*/
var Route$9 = createFileRoute("/api/mcp/tokens")({ server: { handlers: {
	GET: async ({ request }) => {
		try {
			const userId = await requireRequestUserId();
			...
			if (error instanceof UnauthorizedError) return Response.json({ error: "Sign in to manage MCP tokens." }, { status: 401 });
			return Response.json({ error: error instanceof Error ? error.message : "Could not load tokens" }, { status: 400 });
		}
	},
	POST: async ({ request }) => {
		try {
			const limit = checkRateLimit(`mcp-tokens:POST:${clientIp(request)}`, 20, 6e4);
			if (!limit.allowed) return rateLimitedResponse(limit.retryAfterSec);
			const userId = await requireRequestUserId();
			const body = await request.json().catch(() => ({}));
			const created = await createMcpToken(userId, typeof body.name === "string" ? body.name : "");
			return Response.json(created);
		} catch (error) {
			if (error instanceof UnauthorizedError) return Response.json({ error: "Sign in to manage MCP tokens." }, { status: 401 });
			if (error instanceof McpProRequiredError) return Response.json({ error: error.message }, { status: error.status });
			if (error instanceof McpPaidOnlyError) return Response.json({ error: error.message }, { status: error.status });
			return Response.json({ error: error instanceof Error ? error.message : "Could not create token" }, { status: 400 });
		}
	}
} } });
...
var PRO_PLANS = {
	pro_monthly: {
		amount: 1100,
		label: "$11/mo",
		description: "FormaBill Pro — monthly subscription"
	},
	pro_yearly: {
		amount: 9900,
		label: "$99/yr",
		description: "FormaBill Pro — yearly subscription"
	}
};
...
function platformCredentials() {
	const keyId = env$2("RAZORPAY_PLATFORM_KEY_ID");
	const keySecret = env$2("RAZORPAY_PLATFORM_KEY_SECRET");
	return keyId && keySecret ? {
		keyId,
		keySecret
	} : null;
}
function platformBillingConfigured() {
...
*/
async function createPlatformProOrder(userId, plan = "pro_monthly") {
	...
	const response = await fetch("https://api.razorpay.com/v1/orders", {
		...
		body: JSON.stringify({
			...
			notes: {
				userId,
				plan,
				product: "formabill-pro"
			}
		})
	});
	const json = await response.json();
	if (!response.ok || !json.id) throw new Error(json.error?.description || "Could not create the FormaBill Pro order.");
	return {
		configured: true,
		method: "modal",
		orderId: json.id,
...
*/
async function createPlatformProLink(userId, request, plan = "pro_monthly") {
	const selected = PRO_PLANS[plan] ?? PRO_PLANS.pro_monthly;
	const header = platformAuthHeader();
	if (!header) return {
		configured: false,
	...
	const json = await response.json();
	if (!response.ok || !json.short_url) throw new Error(json.error?.description || "Could not create the FormaBill Pro checkout link.");
	return {
		configured: true,
		method: "link",
		id: json.id,
...
async function createPlatformProCheckout(userId, request, plan = "pro_monthly", method = "modal") {
...
async function fetchPlatformOrderNotes(orderId) {
	const header = platformAuthHeader();
	if (!header) return null;
	try {
		const response = await fetch(`https://api.razorpay.com/v1/orders/${encodeURIComponent(orderId)}`, { headers: { Authorization: header.auth } });
		if (!response.ok) return null;
		return (await response.json()).notes ?? null;
	} catch {
		return null;
	}
}
...
*/
async function verifyPlatformPayment(userId, orderId, paymentId, signature) {
	const credentials = platformCredentials();
	...
	return {
		userId,
		plan: notes.plan === "pro_yearly" ? "pro_yearly" : "pro_monthly"
	};
}
function verifyPlatformWebhookSignature(rawBody, signature) {
	const secret = env$2("RAZORPAY_PLATFORM_WEBHOOK_SECRET");
	if (!secret || !signature) return false;
	const expected = createHmac("sha256", secret).update(rawBody).digest("hex");
	try {
...
var Route$8 = createFileRoute("/api/pro/checkout")({ server: { handlers: { POST: async ({ request }) => {
	try {
		const userId = await requireRequestUserId();
		const body = await request.json().catch(() => ({}));
		const checkout = await createPlatformProCheckout(userId, request, isProPlan(body.plan) ? body.plan : "pro_monthly", body.method === "link" ? "link" : "modal");
		return Response.json(checkout, { status: checkout.configured ? 200 : 503 });
	} catch (error) {
		if (error instanceof UnauthorizedError) return Response.json({ error: "Sign in to get Pro — checkout links to your account." }, { status: 401 });
		return Response.json({ error: error instanceof Error ? error.message : "Could not start Pro checkout" }, { status: 400 });
	}
} } } });
var Route$7 = createFileRoute("/api/pro/status")({ server: { handlers: { GET: async ({ request }) => {
	const userId = await getFreeTierUserId();
	const sessionUserId = await getRequestUserId();
	const plan = userId ? await getUserPlan(userId) : {
		isPro: false,
		isLifetimePro: false,
		proPlan: null,
		proExpiresAt: null,
...
*/
var Route$6 = createFileRoute("/api/pro/verify")({ server: { handlers: { POST: async ({ request }) => {
	try {
		const limit = checkRateLimit(`pro-verify:POST:${clientIp(request)}`, 20, 6e4);
		if (!limit.allowed) return rateLimitedResponse(limit.retryAfterSec);
		const userId = await requireRequestUserId();
		const body = await request.json().catch(() => ({}));
		const verified = await verifyPlatformPayment(userId, typeof body.orderId === "string" ? body.orderId : "", typeof body.paymentId === "string" ? body.paymentId : "", typeof body.signature === "string" ? body.signature : "");
		if (!verified) return Response.json({ error: "Payment could not be verified yet — the webhook will activate Pro." }, { status: 202 });
		...
		if (error instanceof UnauthorizedError) return Response.json({ error: "Sign in to verify Pro." }, { status: 401 });
		return Response.json({ error: error instanceof Error ? error.message : "Could not verify payment" }, { status: 400 });
	}
} } } });
var Route$5 = createFileRoute("/api/recurring/process")({ server: { handlers: { POST: async ({ request }) => {
	const userId = await requireFreeTierRequestUserId();
	const all = await listInvoices(userId);
	const now = Date.now();
	const generated = [];
	...
	for (const parent of all) {
		...
		const dueOffset = parent.dueDate ? Math.max(1, Math.round((Date.parse(parent.dueDate) - Date.parse(parent.createdAt)) / 864e5)) : 14;
		const next = {
			...child,
			...
			lineItems: parent.lineItems.map((item) => ({
				...item,
		...
		await upsertInvoice({
			...
			recurrence: {
				...
				nextRunAt: addFrequency((/* @__PURE__ */ new Date()).toISOString(), rec.frequency)
			}
		}, userId);
	}
	return Response.json({
...
var Route$4 = createFileRoute("/api/referrals/attribute")({ server: { handlers: { POST: async ({ request }) => {
	try {
		const limit = checkRateLimit(`referral-attr:POST:${clientIp(request)}`, 20, 6e4);
		if (!limit.allowed) return rateLimitedResponse(limit.retryAfterSec);
		const userId = await requireRequestUserId();
		const body = await request.json().catch(() => ({}));
		...
		if (error instanceof UnauthorizedError) return Response.json({ error: "Sign in first." }, { status: 401 });
		return Response.json({ error: error instanceof Error ? error.message : "Could not apply referral" }, { status: 400 });
	}
} } } });
/** Your referral code, link-ready, plus simple stats. Any signed-in user. */
var Route$3 = createFileRoute("/api/referrals/mine")({ server: { handlers: { GET: async () => {
	try {
		const userId = await requireRequestUserId();
		return Response.json(await getReferralStats(userId));
	} catch (error) {
		if (error instanceof UnauthorizedError) return Response.json({ error: "Sign in to see referrals." }, { status: 401 });
		return Response.json({ error: error instanceof Error ? error.message : "Could not load referrals" }, { status: 400 });
	}
} } } });
var Route$2 = createFileRoute("/api/webhooks/razorpay")({ server: { handlers: { POST: async ({ request }) => {
	const raw = await request.text();
	if (!verifyWebhookSignature(raw, request.headers.get("x-razorpay-signature"))) return Response.json({ error: "Invalid signature" }, { status: 400 });
	let event;
	try {
	...
	const invoiceId = event.payload?.payment_link?.entity?.notes?.invoiceId || event.payload?.payment?.entity?.notes?.invoiceId;
	const linkId = event.payload?.payment_link?.entity?.id;
	const paidEvents = /* @__PURE__ */ new Set([
		"payment_link.paid",
		"payment.captured",
		"order.paid"
	]);
	if (!event.event || !paidEvents.has(event.event)) return Response.json({
		ok: true,
		ignored: event.event
	});
	let invoice = invoiceId ? await markInvoicePaid(invoiceId) : void 0;
	if (!invoice && linkId) {
...
*/
var Route$1 = createFileRoute("/api/webhooks/razorpay-platform")({ server: { handlers: { POST: async ({ request }) => {
	const raw = await request.text();
	if (!verifyPlatformWebhookSignature(raw, request.headers.get("x-razorpay-signature"))) return Response.json({ error: "Invalid signature" }, { status: 400 });
	let event;
	try {
		event = JSON.parse(raw);
	} catch {
	...
	if (!userId || !validPlan(plan)) return Response.json({ error: "Missing Pro checkout metadata" }, { status: 400 });
...
var Route = createFileRoute("/api/mcp/tokens/$id")({ server: { handlers: { DELETE: async ({ params }) => {
	try {
		if (!await revokeMcpToken(await requireRequestUserId(), params.id)) return Response.json({ error: "Token not found." }, { status: 404 });
		return Response.json({ ok: true });
	} catch (error) {
		if (error instanceof UnauthorizedError) return Response.json({ error: "Sign in to manage MCP tokens." }, { status: 401 });
		return Response.json({ error: error instanceof Error ? error.message : "Could not revoke token" }, { status: 400 });
	}
} } } });
...
var ApiAuthStatusRoute = Route$22.update({
	id: "/api/auth-status",
	path: "/api/auth-status",
	getParentRoute: () => Route$30
});
var ApiCheckoutRoute = Route$21.update({
	id: "/api/checkout",
	path: "/api/checkout",
	getParentRoute: () => Route$30
});
var ApiGiftCodesRoute = Route$20.update({
	id: "/api/gift-codes",
	path: "/api/gift-codes",
	getParentRoute: () => Route$30
});
var ApiInvoicesRoute = Route$19.update({
	id: "/api/invoices",
	path: "/api/invoices",
	getParentRoute: () => Route$30
});
var ApiMcpRoute = Route$18.update({
	id: "/api/mcp",
	path: "/api/mcp",
	getParentRoute: () => Route$30
});
...
var AppClientsRoute = Route$16.update({
	id: "/clients",
	path: "/clients",
	getParentRoute: () => AppRoute
});
var AppInvoicesRoute = Route$15.update({
	id: "/invoices",
	path: "/invoices",
	getParentRoute: () => AppRoute
});
...
var InvIdRoute = Route$13.update({
	id: "/inv/$id",
	path: "/inv/$id",
	getParentRoute: () => Route$30
});
var ApiAuthSplatRoute = Route$12.update({
	id: "/api/auth/$",
	path: "/api/auth/$",
	getParentRoute: () => Route$30
});
var ApiGiftCodesRedeemRoute = Route$11.update({
	id: "/redeem",
	path: "/redeem",
	getParentRoute: () => ApiGiftCodesRoute
});
var ApiInvoicesIdRoute = Route$10.update({
	id: "/$id",
	path: "/$id",
	getParentRoute: () => ApiInvoicesRoute
});
var ApiMcpTokensRoute = Route$9.update({
	id: "/tokens",
	path: "/tokens",
	getParentRoute: () => ApiMcpRoute
});
var ApiProCheckoutRoute = Route$8.update({
	id: "/api/pro/checkout",
	path: "/api/pro/checkout",
	getParentRoute: () => Route$30
});
var ApiProStatusRoute = Route$7.update({
	id: "/api/pro/status",
	path: "/api/pro/status",
	getParentRoute: () => Route$30
});
var ApiProVerifyRoute = Route$6.update({
	id: "/api/pro/verify",
	path: "/api/pro/verify",
	getParentRoute: () => Route$30
});
var ApiRecurringProcessRoute = Route$5.update({
	id: "/api/recurring/process",
	path: "/api/recurring/process",
	getParentRoute: () => Route$30
});
var ApiReferralsAttributeRoute = Route$4.update({
	id: "/api/referrals/attribute",
	path: "/api/referrals/attribute",
	getParentRoute: () => Route$30
});
var ApiReferralsMineRoute = Route$3.update({
	id: "/api/referrals/mine",
	path: "/api/referrals/mine",
	getParentRoute: () => Route$30
});
var ApiWebhooksRazorpayRoute = Route$2.update({
	id: "/api/webhooks/razorpay",
	path: "/api/webhooks/razorpay",
	getParentRoute: () => Route$30
});
var ApiWebhooksRazorpayPlatformRoute = Route$1.update({
	id: "/api/webhooks/razorpay-platform",
	path: "/api/webhooks/razorpay-platform",
	getParentRoute: () => Route$30
});
var ApiMcpTokensIdRoute = Route.update({
	id: "/$id",
	path: "/$id",
...
var ApiGiftCodesRouteChildren = { ApiGiftCodesRedeemRoute };
var ApiGiftCodesRouteWithChildren = ApiGiftCodesRoute._addFileChildren(ApiGiftCodesRouteChildren);
var ApiInvoicesRouteChildren = { ApiInvoicesIdRoute };
var ApiInvoicesRouteWithChildren = ApiInvoicesRoute._addFileChildren(ApiInvoicesRouteChildren);
var ApiMcpTokensRouteChildren = { ApiMcpTokensIdRoute };
var ApiMcpRouteChildren = { ApiMcpTokensRoute: ApiMcpTokensRoute._addFileChildren(ApiMcpTokensRouteChildren) };
var rootRouteChildren = {
	...
	ApiMcpRoute: ApiMcpRoute._addFileChildren(ApiMcpRouteChildren),
	InvIdRoute,
	ApiAuthSplatRoute,
	ApiProCheckoutRoute,
	ApiProStatusRoute,
	ApiProVerifyRoute,
	ApiRecurringProcessRoute,
	ApiReferralsAttributeRoute,
	ApiReferralsMineRoute,
	ApiWebhooksRazorpayRoute,
	ApiWebhooksRazorpayPlatformRoute
};
...
//#endregion
export { sendPasswordResetEmail as _, applySettingsToInvoice as a, GROK_PROVIDERS as c, getBaseURL as d, SUPPORT_EMAIL as f, Route$17 as i, GENERIC_OAUTH_ERROR_CODES as l, DEFAULT_SETTINGS as m, Route$13 as n, createDraftInvoice as o, Route$24 as p, Route$14 as r, emptyLineItem as s, router_exports as t, PACKAGE_VERSION as u };
