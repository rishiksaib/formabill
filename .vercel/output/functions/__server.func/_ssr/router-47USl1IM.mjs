...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/router-47USl1IM.js
...
var styles_default = "/assets/styles-BZipLLpm.css";
...
var $$splitComponentImporter$5 = () => import("./terms-FNyM9I3l.mjs");
...
var auth = betterAuth({
	...
	emailAndPassword: {
		...
		sendResetPassword: async ({ user, url }) => {
			const { sendPasswordResetEmail } = await import("./mailer.server-9BxqE6ay.mjs");
...
async function setUserPro(userId, isPro) {
	await (await getSql())`update "user" set "isPro" = ${isPro}, "updatedAt" = CURRENT_TIMESTAMP where "id" = ${userId}`;
}
/** Record a paid checkout plan alongside permanent Pro (webhook action). */
async function setUserProPlan(userId, plan) {
	await (await getSql())`update "user" set "proPlan" = ${plan}, "updatedAt" = CURRENT_TIMESTAMP where "id" = ${userId}`;
}
/**
...
*/
async function maxGiftMonths(userId) {
...
async function createGiftCode(userId, durationMonths) {
	if (!GIFT_DURATIONS.includes(durationMonths)) throw new Error(`"durationMonths" must be one of ${GIFT_DURATIONS.join(", ")}.`);
	const allowed = await maxGiftMonths(userId);
	if (durationMonths > allowed) throw new Error(`You can only gift up to ${allowed} month${allowed === 1 ? "" : "s"} with your current Pro plan.`);
	const sql = await getSql();
...
var Route$20 = createFileRoute("/api/gift-codes")({ server: { handlers: {
	GET: async () => {
		try {
			...
			const [codes, maxMonths] = await Promise.all([listGiftCodes(userId), maxGiftMonths(userId).catch(() => 0)]);
			return Response.json({
				codes,
				maxMonths,
	...
	POST: async ({ request }) => {
		try {
			...
			const created = await createGiftCode(userId, Number(body.durationMonths));
...
async function authenticateMcpToken(token) {
  ...
  `)[0];
	...
	if (!await isProUser(row.userId)) return {
		ok: false,
		reason: "not_pro"
	};
...
var TOOLS = [
	...
	{
		name: "create_invoice",
		description: "Create a draft invoice for the user's client. From-name, currency, tax, and payment methods default from studio settings.",
		inputSchema: {
			type: "object",
		...
		run: async (userId, raw) => {
			...
			return invoiceSummary(await upsertInvoice({
				...draft,
				id: draft.id,
	...
	},
	{
		...
		description: "Create a Razorpay payment link for an invoice. Uses the studio's stored Razorpay keys unless keyId/keySecret are passed. Without keys, returns guidance instead of a link.",
		inputSchema: {
			type: "object",
			required: ["invoiceId"],
			properties: {
		...
		run: async (userId, raw) => {
			...
			if (link.demo) return {
				demo: true,
				message: link.message ?? "Add UPI in Settings or connect Razorpay to collect online payments.",
			...
			return {
				...
				invoice: invoiceSummary(updated)
			};
		}
	},
	{
...
var Route$18 = createFileRoute("/api/mcp")({ server: { handlers: {
	...
	POST: async ({ request }) => {
		...
		if (!token) return Response.json({ error: "Missing MCP token. Send Authorization: Bearer <token> (Settings → AI / MCP)." }, { status: 401 });
		const auth = await authenticateMcpToken(token).catch(() => null);
		if (!auth || !auth.ok) {
			const message = auth?.reason === "not_pro" ? MCP_UPGRADE_MESSAGE : "Invalid or revoked MCP token. Generate a new one in Settings → AI / MCP.";
			return Response.json({ error: message }, { status: auth?.reason === "not_pro" ? 403 : 401 });
		}
		...
		return await handleOne(request, auth.userId, body) ?? new Response(null, { status: 202 });
	}
} } });
var $$splitComponentImporter$4 = () => import("./app-WpSmZCaF.mjs");
var Route$17 = createFileRoute("/app/")({
	validateSearch: (search) => ({ id: typeof search.id === "string" ? search.id : void 0 }),
	component: lazyRouteComponent($$splitComponentImporter$4, "component")
});
var $$splitComponentImporter$3 = () => import("./clients-Ta37nQNR.mjs");
...
var $$splitComponentImporter$1 = () => import("./settings-CuvN8Qoz.mjs");
...
var $$splitComponentImporter = () => import("./inv._id-DhLzSftq.mjs");
